...
module.exports.run = async (bot, message, args) => {
    ...
    console.log(moment().add(args[0], "second"));
    message.delete();


    var timeToWait = moment().add(args[0], "second");
    var verif = false;

    while (!verif) {
        if (moment().format("X") > timeToWait.format("X")) {
            verif = true;
            console.log(messageToSend.fetch());
        }
    }
